...
# ==================== 新增：从PotNet导入势能计算函数 ==================== #
try:
    # 假设 algorithm.py 已被复制到 utils 目录下
    from .algorithm import zeta, exp
except ImportError:
    raise ImportError("请确保 potnet 的 algorithm.py 和编译好的 functions/ 已放置在 utils/ 目录下。")
# ======================================================================= #
...
class MP18(InMemoryDataset):
    ...
    def __init__(self, root='data/', name='MP18', transform=None, pre_transform=[GetY()], r=8.0, n_neighbors=12, edge_steps=50, image_selfloop=True, points=100,target_name="formation_energy_per_atom"):
        ...
        #test_minimal是一个测试用的最小数据集，包含1个样本
    ...
    def get_data_list(self, dict_structures, y):
        ...
        for i, sdict in enumerate(dict_structures):
            ...
            data.glob_feat   = sdict["gatgnn_glob_feat"]
                        # ==================== 新增代码块：集成PotNet的无穷势能特征 ==================== #
            num_atoms = data.n_atoms
            if num_atoms > 1:
                # A. 为长程相互作用创建一个全连接图的边索引 (不包含自环)
                ...
                # B. 准备PotNet算法所需的输入 (必须为 float64 / double)
                cart_coords = pos.numpy().astype(np.double)
                lattice_mat = cell.numpy().astype(np.double)
                
                row, col = inf_edge_index.cpu()
                vecs = cart_coords[row] - cart_coords[col]
                
                # C. 定义势能计算所需的参数 (参考 potnet.yaml)
                infinite_params = [0.5, 3.0, 3.0] # [Coulomb, London, Pauli]
                R_grid = 3

                # D. 调用PotNet的函数计算三种无穷势能加和
                coulomb_pot = zeta(vecs, lattice_mat, param=infinite_params[0], R=R_grid)
                london_pot = zeta(vecs, lattice_mat, param=infinite_params[1], R=R_grid)
                pauli_pot = exp(vecs, lattice_mat, param=infinite_params[2], R=R_grid)
                ...
                # F. 将新计算出的特征附加到Data对象上
                data.inf_edge_index = inf_edge_index
                data.inf_edge_attr = inf_edge_attr
            else: # 处理单原子结构的情况
                data.inf_edge_index = torch.empty((2, 0), dtype=torch.long)
                data.inf_edge_attr = torch.empty((0, 3), dtype=torch.float)
            # ===================== 新增代码块结束 ===================== #
