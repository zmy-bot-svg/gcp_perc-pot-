...
# 替换 utils/dataset_utils.py 中的 get_data_list 方法
    def get_data_list(self, dict_structures, y):
        n_structures = len(dict_structures)
        data_list = []
        logging.info("Getting torch_geometric.data.Data() objects one-by-one for debugging...")
        
        failed_structures = []

        # 使用tqdm来显示总体进度
        for i in tqdm(range(n_structures), desc="Processing Structures"):
            sdict = dict_structures[i]
            target_val = y[i]
            structure_id = sdict.get("structure_id", f"index_{i}")
            
            # 打印当前正在处理的ID，这是调试的关键
            print(f"--> Processing ID: {structure_id}", end='\r')
            ...
            try:
                ...
                cd_matrix, cell_offsets = get_cutoff_distance_matrix(
                    pos, cell, self.r, self.n_neighbors, self.image_selfloop, device=self.device
                )
                edge_indices, edge_weights = dense_to_sparse(cd_matrix)
                data.edge_index, data.edge_attr, data.cell_offsets = edge_indices, edge_weights, cell_offsets
                ...
                # 如果某个结构处理失败，记录下来并继续
                logging.error(f"\n❌ FAILED to process structure ID: {structure_id}. Error: {e}")
                failed_structures.append(structure_id)
                continue
        
        print("\n" + "="*50) # 清除最后一行print
        if failed_structures:
            logging.warning(f"处理失败的结构ID列表: {failed_structures}")
        
        if not data_list:
            raise RuntimeError("All structures failed to process. Please check the errors above.")

        logging.info("Generating node and edge features for successfully processed structures...")
        # 仅对成功处理的结构生成特征
